...
namespace livrocsharp
{
    class funcoesTexto
    {
        static void Main(string[] args)
        {
            ...
            WriteLine("TRIM - retira os espaços em branco antes e depois da expressão");
            WriteLine($"Nome sem espaços: {empresa.Trim()}");

            WriteLine("Lenght - retorna a qtde de caracteres");
            WriteLine($"Tamanho do texto: {empresa.Length}");
            empresa = empresa.Trim();
            WriteLine($"Tamanho do texto após o TRIM: {empresa.Length}");

            WriteLine("ToUpper - converte todos os caracteres para maiúsculas");
            WriteLine($"Converte para maiúsculo: {empresa.ToUpper()}");

            WriteLine("ToLower - converte todos os caracteres para minúsculas");
            WriteLine($"Converte para minúsculas: {empresa.ToLower()}");

            var nomeUpper = "AIRTON SENNA";
            var nomeLower = "airton senna";
            // Comparação 1:
            if (nomeUpper == nomeLower) WriteLine("1 - nomes iguais");
            else WriteLine("1 - nomes diferentes");
            // Comparação 2:
            nomeUpper = nomeUpper.ToLower();
            if (nomeUpper == nomeLower) WriteLine("2 - nomes iguais");
            else WriteLine("2 - nomes diferentes");
            // Comparação 3:
            if(nomeUpper.Equals(nomeLower, StringComparison.OrdinalIgnoreCase)) WriteLine("3 - nomes iguais");
            else WriteLine("3 - nomes diferentes");
            ...
            int qtde = 0;
            foreach (var e in cesta)
            { 
                // p.IndexOf(" ") + 1 retorna a posição inicial 1
                // ex: 5 laranjas = posição 1 + 1 = 2
                WriteLine($"{e.Substring(e.IndexOf(" ") + 1 )}");
                // ler apenas os números para somar na variável qtde:
                qtde += Convert.ToInt32(e.Substring(0, e.IndexOf(" ")));
            }
                WriteLine($"Qtde total : {qtde}");
